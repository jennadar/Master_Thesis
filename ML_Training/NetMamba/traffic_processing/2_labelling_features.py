...
# Example Usage
feature_csv = "D:/jenny/Documents/FAUS_Study/Thesis/My_IoT_23/CTU-IoT-Malware-Capture-1-1/2018-05-09-192.168.100.103.horizontalPort_packet_flow_features.csv"
...
conn_log_labeled = "D:/jenny/Documents/FAUS_Study/Thesis/iot_23_datasets_full/IoTScenarios/CTU-IoT-Malware-Capture-1-1/bro/conn.log.labeled"  # Replace with actual path
output_csv = "D:/jenny/Documents/FAUS_Study/Thesis/My_IoT_23/CTU-IoT-Malware-Capture-1-1/2018-05-09-192.168.100.103.horizontalPort_packet_flow_features_labeled.csv"
...
def map_labels_to_features(feature_csv, log_file, output_csv):
    """Efficiently map labels from conn.log.labeled to features CSV while maintaining time order."""
    
    # Load feature dataset with optimized dtypes
    ...
    # Load labeled log data as a DataFrame
    labeled_df = parse_conn_log_labeled(log_file)

    # Merge based on (ts, src_ip) - keeping original df order
    ...
    # Handle unknown labels efficiently
    df = classify_unknown_labels(df)
...
def classify_unknown_labels(df):
    """Optimized function to classify unknown labels while maintaining sequence order."""
    
    # Step 1: Assign "Benign" for flow_duration > 1 sec
    mask = (df["label"] == "Unknown") & (df["flow_duration"] > 1)
    df.loc[mask, ["label", "detailed_label"]] = ["Benign", "-"]

    # Step 2: Use forward fill only within the same flow
    flow_keys = ["src_ip", "dst_ip", "proto", "src_port", "dst_port"]

    df.sort_values(["src_ip", "dst_ip", "proto", "src_port", "dst_port", "ts"], inplace=True)
    df["label"] = df.groupby(flow_keys)["label"].apply(lambda x: x.ffill().bfill())
    df["detailed_label"] = df.groupby(flow_keys)["detailed_label"].apply(lambda x: x.ffill().bfill())

    # Step 3: Default remaining "Unknown" labels to "Benign"
...
conn_log_labeled = "D:/jenny/Documents/FAUS_Study/Thesis/iot_23_datasets_full/IoTScenarios/CTU-IoT-Malware-Capture-21-1/bro/conn.log.labeled"
